...
abstract class ZabbixApiAbstract
{
    ...
    public function __construct($apiUrl='', $username='', $password='', $httpUser='', $httpPassword='', $authToken='', $sslContext=NULL)
    {
        if($apiUrl)
        ...
        elseif($username && $password)
            $this->userLogin(array('username' => $username, 'password' => $password));
        $this->apiinfoVersion();
    }
    ...
    public function request($method, $params=NULL, $resultArrayKey='', $auth=TRUE)
    {
        ...
        if ($auth && $this->authToken) {
            $gte64 = version_compare($this->version, '6.4', '>=');
            if ($gte64) {
                $headers[] = 'Authorization: Bearer '.$this->authToken;
            } else {
                $this->request['auth'] = $this->authToken;
            }
        }
        // var_dump(array('method' => $method, 'auth' => $auth, 'headers' => $headers));
    ...
    final public function userLogin($params=array(), $arrayKeyProperty='', $tokenCacheDir='/tmp')
    {
        ...
        if($tokenCacheDir && array_key_exists('username', $params) && is_dir($tokenCacheDir))
            $tokenCacheFile = $tokenCacheDir.'/.zabbixapi-token-'.md5($params['username'].'|'.posix_getuid());
    ...
    public function apiinfoVersion($params=array(), $arrayKeyProperty='')
    {
        ...
        // request
        $this->version = $this->request('apiinfo.version', $params, $arrayKeyProperty, $auth);
        return $this->version;
    }
